...
function Fighter(health, x, y, walkAnimation, swingAnimation, deathAnimation, idleAnimation)
{
	...
	this.walk = function(direction)
	{
		...
		{
			if( Math.round(this.sprite.position.x) < Math.round(camera.mouseX) + 4 &&
				Math.round(this.sprite.position.x) > Math.round(camera.mouseX) - 4 &&
			 	Math.round(this.sprite.position.y) < Math.round(camera.mouseY) + 4 &&
				Math.round(this.sprite.position.y) > Math.round(camera.mouseY) - 4)
			{
			...
			{
				this.sprite.attractionPoint(100, camera.mouseX, camera.mouseY);
				this.sword.attractionPoint(50, camera.mouseX, camera.mouseY);
